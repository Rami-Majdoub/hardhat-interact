...
const inputFunctionArgs = async (args: any[]) => {
  ...
  const questions: Array<PromptObject<string>> = args.map((arg) => {
    const { name } = arg;

    const type: keyof typeof solidityTypesToPromptTypes = arg.type;
    const fn =
      solidityTypesToPromptTypes[type] || solidityTypesToPromptTypes._default;
    return fn(name);
  });
    ...
    async (
      ...
      while (!exit) {
        ...
        const abiFunctionDef = contractAbi.filter(
          (funcDef: any) => funcDef.name == funcName
        );
